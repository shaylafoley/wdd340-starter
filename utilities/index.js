...
Util.buildClassificationGrid = async function(data) {
  ...
  if (data.length > 0) {
    ...
    data.forEach(vehicle => { 
      grid += '<li>'
      grid += `<a href="/inv/${vehicle.inv_id}" title="View ${vehicle.inv_make} ${vehicle.inv_model} details">
                <img src="${vehicle.inv_thumbnail}" alt="Image of ${vehicle.inv_make} ${vehicle.inv_model} on CSE Motors">
              </a>`
      grid += '<div class="namePrice">'
      grid += '<hr />'
      grid += `<h2><a href="/inv/${vehicle.inv_id}" title="View ${vehicle.inv_make} ${vehicle.inv_model} details">
